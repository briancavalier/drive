...
- Treat `factory:self-modify` as a high-trust authorization control. Shared logic may apply it only when a trusted operator answers an explicit self-modify approval intervention, and only for the next resumed stage with explicit cleanup afterward. Do not add implicit, default, bulk, or silent preserve-by-default paths for that label.
